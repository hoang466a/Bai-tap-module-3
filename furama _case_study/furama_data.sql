...
employee_salaryluong double not null,
...
add column `ngay_ket_thuc` datetime not null after `ngay_lam_hop_dong`;
...
where `nhan_vien`.`ho_ten` like  "H%" or `nhan_vien`.`ho_ten` like  "T%" or `nhan_vien`.`ho_ten` like  "K%"  and  char_length(ho_ten)<=15;
...
having count(hop_dong.ma_khach_hang)>=1 order by count(hop_dong.ma_khach_hang) asc;
